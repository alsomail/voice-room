...
| **协议路径绑定审计脚本** | **🟢 完成** | **T-0000T：`scripts/audit/protocol-binding-audit.ts` 自动 grep server/client 实现 ↔ TDS 绑定表三角对账，P0 报告阻断合并；输出 JSON/Markdown 报告到 `tests/protocol-audit/`；`npm run audit:protocol` CI 接入；`doc/arch/infra/protocol-binding-audit.md` 脚本契约文档** |
