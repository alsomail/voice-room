...
test('TC-RESILIENCE-00001: WS 断线后自动指数退避重连 + 状态恢复', async ({ e2eEnv }: any) => {
  ...
  try {
    // Round 3 修复：标准化重置（force-stop + am start，不 pm clear 避免弹窗）
    ...
    await loginUser(agent, adbPrefix, phone, phoneLocal);
...
test('TC-RESILIENCE-00005: 进程被系统杀死后冷启动 - JWT 持久化 + 直接进大厅', async ({ e2eEnv }: any) => {
  ...
  try {
    ...
    await loginUser(agent, adbPrefix, phone, phoneLocal);
    await agent.aiWaitFor('大厅房间列表可见', { timeoutMs: 15_000 });
    ...
    const hasConsent2 = await agent.aiBoolean('是否存在数据收集或隐私同意弹窗？');
    if (hasConsent2) await agent.aiTap('"同意" 或 "确定" 按钮');

    await agent.aiWaitFor('页面稳定', { timeoutMs: 10_000 });
