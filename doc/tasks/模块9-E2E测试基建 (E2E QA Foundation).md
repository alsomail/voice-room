...
| **T-0000P** | 基建 | Infra/E2E | Midscene env 注入链补齐（MIDSCENE_MODEL_API_KEY → AI runner） [TDS](../tds/infra/T-0000P.md) | T-0000H | envLoader 增加 Midscene env 读取（`MIDSCENE_MODEL_API_KEY` fallback `OPENAI_API_KEY`）+ .env.example 补充注释 + CI workflow 注入 Secret | 1. 本地设置 MIDSCENE_MODEL_API_KEY 后 WEB 用例正常执行<br>2. 缺失时打印警告并 skip（不阻塞 API/INFRA 测试）<br>3. CI 注入 Secret 后 WEB 用例非 skip<br>4. 示例文件含 3 种部署形态注释 | 2h | Dod | Done | [✅ Passed](../review/模块9-E2E测试基建.md) | [✅ Passed](../../tests/report-20260428-154125/SUMMARY.md) WEB 45/54 tests pass (14/18 cases全绿); TC-AUTH/LOG/WALLET✅; TC-ROOM-00002/00003, TC-GIFT-00002, TC-USER-00002(webkit) 测试代码问题待修 | [⚠️ Conditional](../../tests/report-20260428-154125/SUMMARY.md)（envLoader 本身已 ✅ Passed；9 WEB FAIL pending 由后续 product-manager 拆 T-0000R/S 闭环） |
