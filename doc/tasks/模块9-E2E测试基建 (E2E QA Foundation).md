...
| **T-0000P** | 基建 | Infra/E2E | Midscene env 注入链补齐（MIDSCENE_MODEL_API_KEY → AI runner） [TDS](../tds/infra/T-0000P.md) | T-0000H | envLoader 增加 Midscene env 读取（`MIDSCENE_MODEL_API_KEY` fallback `OPENAI_API_KEY`）+ .env.example 补充注释 + CI workflow 注入 Secret | 1. 本地设置 MIDSCENE_MODEL_API_KEY 后 WEB 用例正常执行<br>2. 缺失时打印警告并 skip（不阻塞 API/INFRA 测试）<br>3. CI 注入 Secret 后 WEB 用例非 skip<br>4. 示例文件含 3 种部署形态注释 | 2h | Dod | Done | [✅ Passed](../review/batch-arch-blockers-infra.md) | - | ⏳ Pending |
| **T-0000Q** | 基建 | Infra/E2E | docker compose preflight 端口冲突检测扩展 [TDS](../tds/infra/T-0000Q.md) | T-0000G | e2e-up.sh 增加端口检测（5432/6379/3000/3001/5173），冲突时打印彩色错误 + 占用进程 PID/名称 + kill 命令提示 | 1. 端口空闲时正常启动<br>2. 冲突时红色错误 + kill 命令提示 + 退出码 1<br>3. 跨平台（macOS lsof / Linux ss）<br>4. 错误信息含进程名称 | 2h | Dod | ✅ Done | [✅ Passed](../review/batch-arch-blockers-infra.md) | - | ⏳ Pending |
