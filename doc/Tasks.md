...
| Task ID | 归属端 | 模块 | 任务名称 | 前置依赖 | 核心描述 | TDD 验收标准 | 状态 | 预估工时 | 负责人 |
|---------|--------|------|----------|----------|----------|-------------|------|----------|--------|
| **T-00017** | App Server | Wallet | 钱包 Schema 与迁移 [TDS](./tds/server/T-00017.md) | T-0000B | users 表增 `diamond_balance BIGINT DEFAULT 0`（CHECK >= 0）+ 新建 `wallet_transactions` 流水表（user_id/type/amount/balance_after/ref_id/reason/created_at） | 1. 迁移脚本可幂等执行<br>2. CHECK 约束防止余额为负<br>3. 流水表带 (user_id, created_at) 索引<br>4. 新注册用户默认 0 | In Progress | 3h | Dod |
