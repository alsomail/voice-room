...
| **T-00101** | Android | data/remote/ws | Android sealed class 反序列化 + RoomViewModel 27+ 野生分支全量迁移 [TDS](../tds/android/T-00101.md) | T-00100 | (a) `WsServerMessage`/`WsClientMessage` sealed class 28 子类型；(b) RuntimeTypeAdapterFactory；(c) RoomViewModel `handleWsMessage` 全量重写消灭 `json.get()`；(d) 集成 OkHttpWebSocketClient；(e) 单测 fixture 由 schemas/ 校验；(f) `?: return` 静默吞错改为 ERROR + 埋点 | 1. PROTO-1~6 + SCHEMA-1 全过<br>2. RoomViewModelTest 全量重写通过<br>3. grep `json.get` 在 RoomViewModel 0 命中 | 14h | Plan | Todo | - | - | ⏳ Pending |
...
| **T-00103** | App Server | ws / dto | App Server 出栈 envelope schema 自检 + HTTP DTO `deny_unknown_fields` [TDS](../tds/server/T-00103.md) | T-00100 | dev/test profile WS 出栈 schema_guard panic；prod ERROR + 埋点；Request DTO 加 `deny_unknown_fields`；ping/pong 兼容期双发 | 1. GUARD-1/2 + DENY-1 + PING-COMPAT 全过<br>2. 现有集成测试 0 回归 | 5h | Plan | Todo | - | - | ⏳ Pending |
...
| **T-00106** | 基建 | Infra/Protocol | 协议字段级 AST 审计（升级 T-0000T）+ CI 阻塞 [TDS](../tds/infra/T-00106.md) | T-00100, T-0000T | 解析 server `json!` 宏 / Android `@SerializedName` / Web Zod；与 schemas/ 字段集合比对；mismatch 报 P0 阻塞 PR | 1. FIELD-1~3 + CI-1 全过<br>2. T-0000T 53 tests 0 回归 | 10h | Plan | Todo | - | - | ⏳ Pending |
...
| **T-00108** | 基建 | Infra/Protocol | `ping`/`pong` → `Ping`/`Pong` 三端同步升级 [TDS](../tds/infra/T-00108.md) | T-00100 | server 出/入栈兼容期双向；Android 心跳改发 PascalCase；fixture 全量改；Web 检查 | 1. PING-1~3 + GREP-1 全过<br>2. 现有心跳测试 0 回归 | 4h | Plan | Todo | - | - | ⏳ Pending |
