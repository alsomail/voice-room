...
| **T-00048** [TDS](../tds/server/T-00048.md) | App Server | Chat | REST/WS Chat 双路径 envelope 等价回归集成测试 | T-00047 | 在 `app/server/tests/chat_dual_path_equivalence.rs` 新增 DUAL-1/2/3 三用例，验证 WS SendMessage ⭐主路径与 REST `POST /api/v1/chat-messages` 备路径产生等价 RoomMessage envelope（除 `envelope.msg_id` UUIDv4 外逐字段断言），含双 WS 观察者并发 + payload 字段逐项校验 + 死连接清理一致性 | 1. 同一 content 通过 WS 与 REST 各发一次，房间内观察到的 envelope 字段全等（除 envelope.msg_id）<br>2. 双路径 `payload.msg_id` 均为合法 UUID 且互不相同<br>3. 死连接清理逻辑在两条路径下行为一致 | 5h | Dod | ✅ Done | [✅ Passed](../review/server-T-00048-chat-dual-path.md) | - | ⏳ Pending |
