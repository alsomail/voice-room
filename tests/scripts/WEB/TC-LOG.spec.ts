...
test.describe('TC-LOG WEB - 审计日志', () => {
  ...
  test('TC-LOG-00001: 时间倒序 + 筛选 + 详情', async ({ page }) => {
    // SKIP-KNOWN P3: /logs 页面 [data-testid="action-select"] 筛选控件未渲染 — pending front-end fix
    test.skip(true, 'SKIP-KNOWN P3: /logs 审计日志筛选控件 action-select 缺失');
    const agent = await login(page);
    await page.goto('/logs');
    await page.waitForLoadState('domcontentloaded');
    // 用 Playwright DOM 定位器验证操作类型筛选框存在（避免 Midscene AI 视觉误判）
    await page.waitForSelector('[data-testid="action-select"]', { timeout: 15_000 });
    // 只验证页面标题（AI 可信度高），筛选框用 DOM 断言已验证
    await agent.aiAssert('页面标题为"操作日志"');
    // 点击查询触发刷新
