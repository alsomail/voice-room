...
| **T-00101** | Android | data/remote/ws | Android sealed class 反序列化 + RoomViewModel 27+ 野生分支全量迁移 [TDS](../tds/android/T-00101.md) | T-00100 | (a) `WsServerMessage`/`WsClientMessage` sealed class 28 子类型；(b) RuntimeTypeAdapterFactory；(c) RoomViewModel `handleWsMessage` 全量重写消灭 `json.get()`；(d) 集成 OkHttpWebSocketClient；(e) 单测 fixture 由 schemas/ 校验；(f) `?: return` 静默吞错改为 ERROR + 埋点 | 1. PROTO-1~6 + SCHEMA-1 全过<br>2. RoomViewModelTest 全量重写通过<br>3. grep `json.get` 在 RoomViewModel 0 命中 | 14h | TDD | In Progress | - | - | ⏳ Pending |
...
| **T-00103** | App Server | ws / dto | App Server 出栈 envelope schema 自检 + HTTP DTO `deny_unknown_fields` [TDS](../tds/server/T-00103.md) | T-00100 | dev/test profile WS 出栈 schema_guard panic；prod ERROR + 埋点；Request DTO 加 `deny_unknown_fields`；ping/pong 兼容期双发 | 1. GUARD-1/2 + DENY-1 + PING-COMPAT 全过<br>2. 现有集成测试 0 回归 | 5h | TDD | In Progress | - | - | ⏳ Pending |
