...
├── lib.rs
├── models/
│   ├── user.rs       # UserModel { id, phone, nickname, avatar, coin_balance, vip_level, ... }
│   └── room.rs       # RoomModel { id, owner_id, title, type, status, ... }
├── jwt/
│   ├── mod.rs
│   ├── encode.rs     # fn encode_jwt(claims, secret) -> Result<String>
│   └── decode.rs     # fn decode_jwt(token, secret) -> Result<Claims>
├── error/
│   └── codes.rs      # 公共错误码常量 (40001, 40101, etc.)
├── crypto/
│   └── password.rs   # fn hash_password(plain) -> String, fn verify_password(plain, hash) -> bool
└── types/
    └── mod.rs         # UserId(Uuid), RoomId(Uuid), AdminId(Uuid) 等新类型
```
