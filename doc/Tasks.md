...
| Task ID | 归属端 | 模块 | 任务名称 | 前置依赖 | 核心描述 | TDD 验收标准 | 状态 | 预估工时 | 负责人 | UI设计文档 |
|---------|--------|------|----------|----------|----------|-------------|------|----------|--------|------------|
| **T-20014** | Web | Governance | 房间治理日志查询页 [TDS](./tds/web/T-20014.md) | T-10016, T-20007 | 新菜单 `/governance/logs`：筛选栏（房间/操作者/目标/类型/时间区间）+ 表格（时间/类型/房间/操作者/目标/原因/时长）+ CSV 导出 + 分页；用户详情页可跳转到此页预填目标 user_id | 1. 默认查最近 7 天<br>2. type 下拉 [全部/踢出/禁麦/禁言]<br>3. 操作者点击可跳转该管理员详情页<br>4. CSV 导出当前筛选结果<br>5. 空数据空状态占位 | Review | 5h | TDD（R1 未通过） | 26 tests passed |
