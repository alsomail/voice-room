...
> **当前状态机**：负责人 [TDD] | 状态 [❌ Failed] | 修复轮次 [5/10] (Android Round 5 — 发现 4 个新 P0，P0-2 修复不完整)
>
> **历史归档**：Admin Server 子批次 (T-10025~28, T-10030~32) 已于 Round 2 审查通过 ✅ Passed。Web Admin 子批次 (T-20030~36) 已于 Round 2 审查通过 ✅ Passed。Android 子批次 (T-30060~65, T-30070~75) Round 1~3 P0 均已修复，Round 4 开批。
...
> **批次说明**：本批次为模块10 (T-30060~65) 与模块11 (T-30070~75) 的 Android 客户端代码审查。
> Admin Server (T-10025~28, T-10030~32) 与 Web Admin (T-20030~36) 已在上文 §2/§4 完成并通过审查 ✅ Passed。

- **包含任务**：
  - [模块 10: Google Play 真支付 (E-08)](../tasks/模块10-Google%20Play%20真支付%20(E-08).md) Android 部分
    - [T-30060](../tds/android/T-30060.md) 充值页 UI（SKU 列表）
    - [T-30061](../tds/android/T-30061.md) Billing 防腐层 (BillingPort)
    - [T-30062](../tds/android/T-30062.md) 创建订单 + 唤起 Billing
    - [T-30063](../tds/android/T-30063.md) 客户端 verify + ack + 容错
    - [T-30064](../tds/android/T-30064.md) 充值历史页
    - [T-30065](../tds/android/T-30065.md) Dev/Staging 开发者菜单 [Dev-only]
  - [模块 11: 贵族体系 (E-09)](../tasks/模块11-贵族体系%20(E-09).md) Android 部分
    - [T-30070](../tds/android/T-30070.md) 贵族中心页
    - [T-30071](../tds/android/T-30071.md) 钻石/真金购买流程
    - [T-30072](../tds/android/T-30072.md) 进场特效播放器
    - [T-30073](../tds/android/T-30073.md) 用户贵族徽章组件
    - [T-30074](../tds/android/T-30074.md) 资料卡贵族框
    - [T-30075](../tds/android/T-30075.md) 续费/过期/失败提醒
- **关联协议**：[payment_api.md](../protocol/payment_api.md) | [nobility_api.md](../protocol/nobility_api.md) | [websocket_signals.md](../protocol/websocket_signals.md)
- **代码基线**：Kotlin 编译零错误（`./gradlew :app:compileLocalDebugKotlin` BUILD SUCCESSFUL）
- **开始时间**：2026-05-15
...
> 从各 Android Task TDS 第二节「协议路径绑定表」合并，作为 global-code-reviewer P0 必查项输入。

### HTTP REST — 支付模块 (T-30060~65)

| # | Task | Method + Path | 客户端调用方 (Android) | 服务端处理函数 | protocol/ 锚点 |
|---|------|---------------|----------------------|---------------|---------------|
| 1 | T-30060 | `GET /api/v1/payments/skus` ⭐ | `SkuApi.kt::getSkus()` | server payment::controller::list_skus_handler | [payment_api.md §1](../protocol/payment_api.md) |
| 2 | T-30062 | `POST /api/v1/payments/orders` ⭐ | `OrderApi.kt::createOrder` | T-00051 create_order_handler | [payment_api.md §3.1](../protocol/payment_api.md) |
| 3 | T-30063 | `POST /api/v1/payments/google/verify` ⭐ | `VerifyApi.kt::verify` | T-00052 verify_handler | [payment_api.md §3.2](../protocol/payment_api.md) |
| 4 | T-30064 | `GET /api/v1/wallet/transactions?type=recharge` ⭐ | `HistoryApi.kt::list` | T-00018 list_transactions | [data_models.md](../protocol/data_models.md) |
| 5 | T-30064 | `POST /api/v1/payments/google/verify` ⭐ | `HistoryApi.kt::retryVerify` | T-00052 verify_handler | [payment_api.md §3.2](../protocol/payment_api.md) |
| 6 | T-30065 | `POST /api/v1/_dev/mock_recharge` ⭐ | `MockRechargeApi.kt::mock` | T-00055 mock_recharge_handler | [payment_api.md §6](../protocol/payment_api.md) |

### HTTP REST — 贵族模块 (T-30070~75)

| # | Task | Method + Path | 客户端调用方 (Android) | 服务端处理函数 | protocol/ 锚点 |
|---|------|---------------|----------------------|---------------|---------------|
| 7 | T-30070 | `GET /api/v1/nobles/tiers` ⭐ | `NobleApi.kt::getTiers` | T-00066 list_tiers_handler | [nobility_api.md §10.3.1](../protocol/nobility_api.md) |
| 8 | T-30070 | `GET /api/v1/nobles/me` ⭐ | `NobleApi.kt::getMe` | T-00066 get_me_handler | [nobility_api.md §10.3.2](../protocol/nobility_api.md) |
| 9 | T-30071 | `POST /api/v1/nobles/purchase` ⭐ | `NobleApi.kt::purchase` | T-00067 purchase_handler | [nobility_api.md §10.3.3](../protocol/nobility_api.md) |
| 10 | T-30071 | `POST /api/v1/nobles/usd_purchase_intent` ⭐ | `NobleApi.kt::usdPurchaseIntent` | T-00067 intent handler | [nobility_api.md §10.3.4](../protocol/nobility_api.md) |
| 11 | T-30075 | `POST /api/v1/nobles/me/renew_reminder/ack` ⭐ | `AckRepository.kt::ack` | T-00066/68 ack handler | [nobility_api.md §10.3.6](../protocol/nobility_api.md) |

### WebSocket S->C / S->Room — 支付模块

| # | Task | 信令 | 客户端接收方 (Android) | protocol/ 锚点 |
|---|------|------|----------------------|---------------|
| 1 | T-30063 | `BalanceUpdated` (reason=`payment_credit`) | `WsClient` -> WalletViewModel.onBalanceUpdated | [websocket_signals.md §6.13.1](../protocol/websocket_signals.md) |

### WebSocket S->C / S->Room — 贵族模块

| # | Task | 信令 | 客户端接收方 (Android) | protocol/ 锚点 |
|---|------|------|----------------------|---------------|
| 2 | T-30071 | `NobleChanged` | WsClient -> PurchaseViewModel.onNobleChanged | [nobility_api.md §10.4.1](../protocol/nobility_api.md) |
| 3 | T-30071 | `BalanceUpdated` | WsClient -> WalletRepository | [websocket_signals.md §6.14.2](../protocol/websocket_signals.md) |
| 4 | T-30072 | `NobleEntered` ⭐ | RoomViewModel.onNobleEntered | [nobility_api.md §10.4.5](../protocol/nobility_api.md) |
| 5 | T-30072 | `NobleEntranceGlobal` | GlobalEventBus.onNobleEntranceGlobal -> MarqueeOverlay | [nobility_api.md §10.4.6](../protocol/nobility_api.md) |
| 6 | T-30075 | `NobleRenewFailed` | NobleNotifier.onRenewFailed | [nobility_api.md §10.4.3](../protocol/nobility_api.md) |
| 7 | T-30075 | `NobleExpired` | NobleNotifier.onExpired | [nobility_api.md §10.4.4](../protocol/nobility_api.md) |
| 8 | T-30075 | `NobleRenewSuccess` | NobleNotifier.onRenewSuccess | [nobility_api.md §10.4.2](../protocol/nobility_api.md) |

### 渲染层字段来源 (T-30073/74)

| # | Task | 字段来源 | 用途 |
|---|------|---------|------|
| 1 | T-30073 | `MemberSnapshot[].noble` / `UserJoined.noble` / `RoomMessage.sender.noble` | NobleBadge Composable 渲染 |
| 2 | T-30074 | `noble.frame_url` (来自 server 下发) | AvatarWithFrame 头像框渲染 |

### P0 审查要点

> global-code-reviewer 须逐条对账：
> 1. Android Retrofit API 路径与 Server 路由注册完全一致
> 2. WS 信令处理函数名与 Server 下发信令名一致
> 3. BillingClient 防腐层隔离：业务层无 `com.android.billingclient` 直接 import
> 4. 生产环境 DI 注入链完整（AppContainer.fromBuildConfig 正确注入 GooglePlayBillingAdapter）
> 5. createOrderAndPay() 完整流：connect -> launchBillingFlow -> collect purchaseResults -> verify -> acknowledge
...
> 第 1~3 轮审查在 v3.35 前完成。三轮审查共发现 P0 缺陷数个，均已由 TDD 修复。

**Round 1 (4 P0 + 3 P1) — 已全部修复**
**Round 2 (P0: BillingClient 回调桥接) — 已修复：**
- P0: BillingClient `onPurchasesUpdated` 回调未桥接到 Kotlin 层 -> 通过 `IBillingPort.purchaseResults: SharedFlow<PurchaseResult>` 修复。

**Round 3 (P0: 生产环境 GooglePlayBillingAdapter 未注入) — 已修复：**
- P0: `GooglePlayBillingAdapter` 在生产环境未实例化/未注入到 `RechargeViewModel`，真支付链路完全断裂 -> 通过 `AppContainer.fromBuildConfig()` 构造修复，`billingPort` 改为非空。
...
| 验证项 | 状态 | 说明 |
|--------|------|------|
| `./gradlew :app:compileLocalDebugKotlin` | ✅ BUILD SUCCESSFUL | 编译零错误 |
| `IBillingPort.purchaseResults: SharedFlow<PurchaseResult>` | ✅ 已实现 | Round 2 P0 修复 |
| `AppContainer.billingPort` 在 `fromBuildConfig` 注入 | ✅ 已注入 | Round 3 P0 修复 |
| `AppContainer.billingPort` 在 `forUnitTest` 注入 | ✅ 已注入 | Round 3 P0 修复 |
| `RechargeScreen` 传递 `billingPort + PendingPurchaseHandler` 给 ViewModel | ✅ 已实现 | DI 链完整 |
| `createOrderAndPay()` 完整流 | ✅ 已实现 | connect -> launchBillingFlow -> collect purchaseResults -> verify -> acknowledge |

---

#### 已知 P1 遗留（本轮不阻塞）

以下 P1 级别问题已在前期审查中识别，本轮维持非阻塞状态：

- [ ] **P1-1**: `SharedFlow.collect` 无限监听 — 无生命周期感知取消机制，可能导致 coroutine 泄漏
- [ ] **P1-2**: `NobleEntrancePlayer` Lottie 集成 — 进场特效播放器 Lottie 动画未完成端到端集成
- [ ] **P1-3**: `NobleBadge` 硬编码颜色 — 徽章组件可能硬编码 6 档颜色而非从 server `badge_color` 字段读取
- [ ] **P1-4**: `MyNoble` 字段跨引用 — 贵族中心页 `MyNoble` 数据模型可能跨模块引用支付模块字段
- [ ] **P1-5**: `DevToolsScreen` 按钮未对齐 TDS — Dev 菜单按钮标签/行为与 TDS 设计文档不完全一致
- [ ] **P1-6**: `PendingPurchaseHandler` retry-count 竞态 — 重试计数器无同步保护，并发场景可能计数不准确

**P1 处置策略**：以上 6 项 P1 不阻塞本轮放行，建议后续迭代（Phase 1.8）统一修复。

---

#### 协议路径对账 (P0 必查项)

##### HTTP REST — 11 条路径逐条对账

对 11 条 Android HTTP REST 路径与 App Server 路由注册双向对账：

```bash
# Android 调用方 vs Server 路由注册对账
grep -n "@GET\|@POST" app/android/.../PaymentApiService.kt app/android/.../NobilityApiService.kt
grep -rn "\.route.*payments/skus\|\.route.*nobles/tiers\|\.route.*nobles/me\|\.route.*nobles/purchase\|\.route.*wallet/transactions\|\.route.*_dev/mock" app/server/src/
```

| # | Task | Method + Path | Android 调用方 | Server Handler | 对账 |
|---|------|---------------|---------------|---------------|------|
| 1 | T-30060 | `GET /api/v1/payments/skus` ⭐ | `PaymentApiService.listSkus()` → `@GET("payments/skus")` | `list_skus_handler` (payment/controller.rs) | ✅ |
| 2 | T-30062 | `POST /api/v1/payments/orders` ⭐ | `PaymentApiService.createOrder()` → `@POST("payments/orders")` | `create_order_handler` (payment/controller.rs) | ✅ |
| 3 | T-30063 | `POST /api/v1/payments/google/verify` ⭐ | `PaymentApiService.verifyPurchase()` → `@POST("payments/google/verify")` | `verify_handler` (payment/controller.rs) | ✅ |
| 4 | T-30064 | `GET /api/v1/wallet/transactions?type=recharge` ⭐ | `WalletApiService.listTxns(page,size,type)` → `@GET("wallet/transactions")` | `list_transactions` (wallet/controller.rs) | ✅ (路径匹配；但调用未传 type=recharge，见缺陷 P0-2) |
| 5 | T-30064 | `POST /api/v1/payments/google/verify` ⭐ | 同 #3（PendingPurchaseHandler.retryPending 调用 `paymentRepo.verifyPurchase`） | 同 #3 | ✅ |
| 6 | T-30065 | `POST /api/v1/_dev/mock_recharge` ⭐ | **无独立 MockRechargeApi**；DevToolsScreen 使用 `paymentRepo.createOrder("diamond_300")` | `mock_recharge_handler` (payment/controller.rs:175) | ❌ **路径不匹配** — 见缺陷 P1-3 |
| 7 | T-30070 | `GET /api/v1/nobles/tiers` ⭐ | `NobilityApiService.listTiers()` → `@GET("nobles/tiers")` | `list_tiers_handler` (nobility/controller.rs) | ✅ |
| 8 | T-30070 | `GET /api/v1/nobles/me` ⭐ | `NobilityApiService.getMyNoble()` → `@GET("nobles/me")` | `get_me_handler` (nobility/controller.rs) | ✅ |
| 9 | T-30071 | `POST /api/v1/nobles/purchase` ⭐ | `NobilityApiService.purchase()` → `@POST("nobles/purchase")` | `purchase_handler` (nobility/controller.rs) | ✅ |
| 10 | T-30071 | `POST /api/v1/nobles/usd_purchase_intent` ⭐ | **未实现** — NobilityApiService 无此端点；NobleCenterViewModel.purchaseWithUsd() 为 TODO 占位回退 | **Server 也未实现** | ❌ **双向缺失** — 见缺陷 P1-4 |
| 11 | T-30075 | `POST /api/v1/nobles/me/renew_reminder/ack` ⭐ | **未实现** — 无 AckRepository 或对应 API 方法 | **Server 也未实现** | ❌ **双向缺失** — 见缺陷 P1-5 |

**HTTP REST 对账结论**：9 条路径完全匹配，1 条路径不匹配（T-30065 mock_recharge），2 条双向缺失（T-30071 usd_purchase_intent、T-30075 renew_reminder/ack）。

---

##### WebSocket 信令 — 9 条信号逐条对账

对 9 条 WS S→C / S→Room 信号进行 App Server 发送端 ↓ Android 接收端双向对账：

```bash
# Server 发送端验证（所有 8 个贵族信号均已实现）
grep -rn "NobleChanged\|NobleEntered\|NobleEntranceGlobal\|NobleRenewSuccess\|NobleRenewFailed\|NobleExpired" app/server/src/ --include="*.rs"

# Android 接收端验证
grep -rn "NobleChanged\|NobleEntered\|NobleEntranceGlobal\|NobleRenewSuccess\|NobleRenewFailed\|NobleExpired\|BalanceUpdated" app/android/app/src/main/java/ --include="*.kt"
```

| # | Task | 信令 | Server 发送 | Android 接收 | 对账 |
|---|------|------|-----------|------------|------|
| 1 | T-30063 | `BalanceUpdated` (reason=`payment_credit`) | ✅ (payment controller → WS unicast) | ✅ `WalletViewModel.handleWsMessage()` 解析 JSON type==BalanceUpdated → `payload.diamond_balance` | ✅ |
| 2 | T-30071 | `NobleChanged` | ✅ (nobility/controller.rs:186 — S→C unicast + S→Room broadcast) | ❌ **无处理器** — 全文 grep 零命中 | ❌ P0 |
| 3 | T-30071 | `BalanceUpdated` | ✅ (cron.rs / purchase flow) | ✅ 同 #1，WalletViewModel 和 GiftPanelViewModel 均处理 | ✅ |
| 4 | T-30072 | `NobleEntered` ⭐ | ✅ (room/handler/mod.rs:2499 — S→Room broadcast LV3+) | ❌ **无处理器** — RoomViewModel 无 noble 相关代码 | ❌ P0 |
| 5 | T-30072 | `NobleEntranceGlobal` | ✅ (room/handler/lifecycle.rs:297 — S→Global broadcast LV5+) | ❌ **无处理器** — 无 GlobalEventBus/MarqueeOverlay 实现 | ❌ P0 |
| 6 | T-30075 | `NobleRenewFailed` | ✅ (nobility/cron.rs:70 — S→C unicast) | ✅ `NobleRenewalListener` — 但使用 `text.contains("NobleRenewFailed")` 字符串匹配（非 JSON 解析） | ⚠️ P1（功能可用，实现脆弱）|
| 7 | T-30075 | `NobleExpired` | ✅ (nobility/cron.rs:104 — S→C unicast) | ✅ `NobleRenewalListener` — 同上 `text.contains("NobleExpired")` | ⚠️ P1（同上）|
| 8 | T-30075 | `NobleRenewSuccess` | ✅ (nobility/cron.rs:60 — S→C unicast) | ❌ **无处理器** — NobleRenewalListener 仅处理 Failed/Expired | ❌ P0 |

**WS 信令对账结论**：4 条信号完全缺失 Android 处理器（NobleChanged、NobleEntered、NobleEntranceGlobal、NobleRenewSuccess），2 条使用脆弱字符串匹配（NobleRenewFailed、NobleExpired），3 条正确处理（BalanceUpdated ×2 + 同一条）。
...
```bash
# 验证 com.android.billingclient 仅出现在 GooglePlayBillingAdapter.kt
grep -rn "com.android.billingclient" app/android/app/src/main/java/ --include="*.kt"
```

**结果**：
- `GooglePlayBillingAdapter.kt:5` — `import com.android.billingclient.api.*` ✅ (唯一合法位置)
- `IBillingPort.kt:8` — 注释文字，非 import ✅

**防腐层结论**：✅ 边界完全隔离。业务层（RechargeViewModel、RechargeScreen、AppContainer）均仅通过 `IBillingPort` 接口依赖，无任何 `com.android.billingclient` 直接引用。
...
```bash
# 从 AppContainer → RechargeScreen → RechargeViewModel → BillingPort 完整链路
grep -n "billingPort\|GooglePlayBillingAdapter\|FakeBillingPort" app/android/.../AppContainer.kt
```

| 链路节点 | 代码位置 | 验证 |
|---------|---------|------|
| 生产注入 | `AppContainer.fromBuildConfig:338` — `billingPort = GooglePlayBillingAdapter(appCtx)` | ✅ 非空 |
| 测试注入 | `AppContainer.forUnitTest:480` — `billingPort = FakeBillingPort()` | ✅ 非空 |
| Screen 传递 | `RechargeScreen.kt:39-43` — `factory = RechargeViewModel.factory(..., billingPort = container.billingPort, ...)` | ✅ |
| ViewModel 消费 | `RechargeViewModel.kt:19` — `private val billingPort: IBillingPort` | ✅ 构造函数注入 |

**DI 链路结论**：✅ 完整可追踪，生产/测试两路径均正确注入非空实例。

---

##### 前置 P0 修复验证 (6 项)

| # | P0 缺陷（Round 1-3）| 验证位置 | 状态 |
|---|-------------------|---------|------|
| 1 | NobleRenewalListener WS 未实现 | `NobleRenewalListener.kt:37-68` — LaunchedEffect 监听 wsClient.state | ✅ 已实现 |
| 2 | RechargeViewModel 未调用 BillingPort | `RechargeViewModel.kt:65-122` — createOrderAndPay() 完整流 | ✅ 已实现 |
| 3 | VerifyResponse 字段名错误 new_state→state | `PaymentApiService.kt:59-63` — `VerifyResponse(state=..., diamonds_credited=...)` | ✅ 已修复 |
| 4 | DevToolsScreen 裸 OkHttp | `DevToolsScreen.kt:74` — `container.paymentRepository` (IPaymentRepository 接口) | ✅ 已改用 Repository |
| 5 | BillingClient 回调未桥接 | `IBillingPort.kt:33` — `val purchaseResults: SharedFlow<PurchaseResult>` | ✅ SharedFlow 已定义 |
| 6 | 生产环境 GooglePlayBillingAdapter 未注入 | `AppContainer.kt:338` — `billingPort = GooglePlayBillingAdapter(appCtx)` | ✅ 非空注入 |

**前置 P0 修复验证结论**：✅ 6 项全部正确修复，无回归。

---

#### 缺陷清单

- [ ] **缺陷 1**：[级别 P0] **`BalanceDto` 字段名与 Server `BalanceResponse` 不匹配 — HTTP 余额查询永远返回 0**

  - **文件与行号**：
    - `app/android/.../data/remote/model/WalletModels.kt:8-10` — `BalanceDto(@SerializedName("balance") val balance: Long)`
    - `app/server/src/modules/wallet/dto.rs:15-17` — `BalanceResponse { pub diamond_balance: i64 }`
    - `app/android/.../data/wallet/RetrofitWalletRepository.kt:32-36` — `getBalance()` 返回 `data.balance`
    - `app/android/.../feature/recharge/RechargeViewModel.kt:49-56` — `loadBalance()` 依赖 HTTP 余额

  - **问题说明**：
    - Server 序列化 `BalanceResponse` → JSON `{"diamond_balance": 1234}`
    - Android `BalanceDto` 声明 `@SerializedName("balance")`，Gson 反序列化时查找 key `"balance"` → 未找到 → `balance` 默认为 `0L`
    - 影响链路：
      1. `RechargeViewModel.init()` 调用 `loadBalance()` → `walletRepo.getBalance()` → HTTP `GET /wallet/balance` → 永远返回 `Result.success(0L)`
      2. `RechargeUiState.balance` 初始为 0，UI 显示 "0 💎"
      3. 仅在 WS `BalanceUpdated` 信令抵达后余额才被修正为正确值
      4. 若 WS 连接延迟/失败，用户将一直看到错误余额 0
      5. `WalletViewModel` 同理受影响（`WalletScreen` 余额卡片始终 0 直到 WS 事件到达）
    - 注意：WS `BalanceUpdated` 处理中 `WalletViewModel.handleWsMessage()` 正确读取 `payload.get("diamond_balance")`（line 172），故 WS 路径不受影响。

  - **修复建议**：将 `BalanceDto` 的 `@SerializedName` 改为 `"diamond_balance"`：
    ```kotlin
    data class BalanceDto(
        @SerializedName("diamond_balance") val balance: Long,
    )
    ```
    同时验证 `WalletViewModel` WS 处理路径（`payload.get("diamond_balance")`）与服务端 WS 信令 schema 字段名一致。

  - **TDD 修复记录**：[等待 TDD 填写修复逻辑与 Commit ID]

- [ ] **缺陷 2**：[级别 P0] **4 个贵族 WS 信令 Android 端处理器完全缺失 — Server 已广播但客户端无人接收**

  - **文件与行号**：以下 4 个信令在 `app/android/app/src/main/java/` 全目录 grep 零命中：
    - `NobleChanged` — Server `nobility/controller.rs:186` 发送 S→C + S→Room 广播
    - `NobleEntered` — Server `room/handler/mod.rs:2499` 发送 S→Room 广播
    - `NobleEntranceGlobal` — Server `room/handler/lifecycle.rs:297` 发送 S→Global 广播
    - `NobleRenewSuccess` — Server `nobility/cron.rs:60` 发送 S→C 单播

  - **问题说明**：
    - App Server 已在 Round 2~3 完整实现了全部 8 个贵族 WS 信令的广播逻辑，但 Android 端仅接收其中 3 个（`BalanceUpdated`、`NobleRenewFailed`、`NobleExpired`）
    - **缺失影响**（按信号）：
      | 信号 | 缺失影响 |
      |------|---------|
      | `NobleChanged` | 贵族购买/升级/降级后，UI 不更新贵族等级、徽章颜色、权限；用户需手动退出重进才能看到变化 |
      | `NobleEntered` | LV3+ 用户进入房间时，其他用户看不到进场特效（`NobleEntrancePlayer` 虽已实现但无事件源驱动） |
      | `NobleEntranceGlobal` | LV5+ 公爵/国王进场时，全服跑马灯不显示 |
      | `NobleRenewSuccess` | 自动续费成功后无 UI 通知，用户不知道续费已成功（`NobleRenewalListener` 仅处理失败/过期） |
    - NobleRenewalListener（`NobleRenewalListener.kt`）当前仅处理 `NobleRenewFailed` 和 `NobleExpired`，使用脆弱的 `text.contains()` 字符串匹配而非 JSON 解析。该文件是扩展上述 4 个信号处理器的自然位置。

  - **修复建议**（优先级排序）：
    1. **NobleRenewSuccess**（P0）：在 `NobleRenewalListener.kt` 中新增 `text.contains("NobleRenewSuccess")` 分支 → 弹出 Toast "续费成功" 或静默更新 UI
    2. **NobleChanged**（P0）：在 `NobleCenterViewModel` 或新建 `NobleChangedListener` 中订阅 `wsClient.state`，解析 `NobleChanged` 事件 → 调用 `loadMyNoble()` 刷新贵族状态；同时在房间内消费此事件以更新 `UserJoined.noble` / `MemberSnapshot.noble` 字段
    3. **NobleEntered**（P0）：在 `RoomViewModel` 的 `handleWsMessage()` 或等效位置解析 `NobleEntered` 事件 → 将 `NobleEntrance` 对象入队到 `EntranceQueueController`
    4. **NobleEntranceGlobal**（P0）：新建 `GlobalEventBus` 单例，订阅 `wsClient.state` → 解析 `NobleEntranceGlobal` 事件 → 驱动 `MarqueeOverlay` Composable

  - **TDD 修复记录**：[等待 TDD 填写修复逻辑与 Commit ID]

- [ ] **缺陷 3**：[级别 P1] **`NobleRenewalListener` 使用脆弱的 `text.contains()` 字符串匹配检测 WS 信令**

  - **文件与行号**：`app/android/.../feature/noble/NobleRenewalListener.kt:44,56`
  - **问题说明**：当前使用 `text.contains("NobleRenewFailed")` 和 `text.contains("NobleExpired")` 判断 WS 信令类型。此方式存在以下风险：
    1. 若聊天消息或用户昵称包含 "NobleRenewFailed" 字符串 → 误触发弹窗
    2. 若 Server 未来在 payload 中嵌入嵌套 JSON 含这些字符串 → 误触发
    3. 无法解析 payload 中的结构化字段（如 `reason`、`days_remaining`），当前 `RenewFailedEvent` 通过 Gson 二次解析
    4. 与 `WalletViewModel.handleWsMessage()` 的 JSON 解析方式不一致
  - **修复建议**：统一为 JSON 解析方式，参照 `WalletViewModel:167-172` 模式：
    ```kotlin
    val json = JsonParser.parseString(text)?.asJsonObject ?: return
    when (json.get("type")?.asString) {
        "NobleRenewFailed" -> { /* parse payload */ }
        "NobleExpired" -> { /* parse payload */ }
        "NobleRenewSuccess" -> { /* new handler */ }
    }
    ```
    本文缺陷 2 中新增的信号处理器（NobleChanged/NobleEntered/NobleEntranceGlobal）也应按此模式实现。

  - **TDD 修复记录**：[等待 TDD 填写修复逻辑与 Commit ID]

- [ ] **缺陷 4**：[级别 P1] **`RechargeHistoryScreen` 未使用服务端 `type=recharge` 过滤 — 全量拉取 + 客户端过滤**

  - **文件与行号**：
    - `app/android/.../feature/recharge/RechargeHistoryViewModel.kt:32` — `walletRepo.listTxns(1, 50)` 未传 type 参数
    - `app/android/.../feature/recharge/RechargeHistoryViewModel.kt:34-36` — 客户端过滤 `it.reason.contains("recharge") || it.amount > 0`
    - `app/android/.../data/remote/model/WalletModels.kt:15-21` — `TxnDto` 缺少 `txn_type` 字段
    - `app/server/src/modules/wallet/dto.rs:43` — Server 支持 `?type=recharge` 过滤

  - **问题说明**：
    1. `RechargeHistoryViewModel.loadHistory()` 调用 `walletRepo.listTxns(1, 50)` 不传 `type` 参数，拉取**全量**流水记录（包含送礼、退款、管理调整等）
    2. 然后在客户端用 `reason.contains("recharge", ignoreCase = true) || amount > 0` 粗糙过滤
    3. 根本原因：`TxnDto` 未映射 Server 返回的 `type` 字段（`WalletTxnType` 枚举，如 `"recharge"` / `"gift_send"` / `"noble_purchase"` 等）
    4. 客户端过滤漏掉 `reason` 不含 "recharge" 的充值交易，且 `amount > 0` 误纳入礼物收入等非充值流水
  - **修复建议**：
    1. 在 `TxnDto` 中新增 `@SerializedName("type") val txnType: String?` 字段
    2. 在 `WalletTxn` 领域模型中新增 `val txnType: String?` 字段
    3. 在 `RechargeHistoryViewModel.loadHistory()` 中改为 `walletRepo.listTxns(1, 50, type = "recharge")`
    4. 移除客户端过滤逻辑
    5. 同步更新 `RetrofitWalletRepository.listTxns()` 签名支持 `type` 参数透传至 `WalletApiService`

  - **TDD 修复记录**：[等待 TDD 填写修复逻辑与 Commit ID]

- [ ] **缺陷 5**：[级别 P2] **`DevToolsScreen` 使用常规支付流程替代专用 `POST /api/v1/_dev/mock_recharge` 端点**

  - **文件与行号**：
    - `app/android/.../feature/devtools/DevToolsScreen.kt:74-98` — 调用 `paymentRepo.createOrder("diamond_300")`
    - `app/server/src/modules/payment/controller.rs:169-175` — `mock_recharge_handler` 已实现
    - 协议绑定表 §5 #6：`MockRechargeApi.kt::mock` → `POST /api/v1/_dev/mock_recharge`

  - **问题说明**：协议绑定表要求专用 MockRechargeApi 调用 `POST /api/v1/_dev/mock_recharge`，但 DevToolsScreen 复用了生产支付流程 `createOrder → launchBillingFlow`。在 dev/staging 环境中，如果 BillingClient 不可用（模拟器无 Google Play），DevToolsScreen 的充值按钮将失败。专用 mock 端点可绕过 BillingClient 直接在服务端模拟充值。
  - **修复建议**：新建 `MockRechargeApi` Retrofit 接口，在 `PaymentApiService` 或独立文件中定义 `@POST("_dev/mock_recharge") suspend fun mockRecharge(...)`；DevToolsScreen 优先使用此端点。

  - **TDD 修复记录**：[等待 TDD 填写修复逻辑与 Commit ID]

- [ ] **缺陷 6**：[级别 P2] **`NobleCenterViewModel.purchaseWithUsd()` 为 TODO 占位，回退到钻石购买**

  - **文件与行号**：`app/android/.../feature/noble/NobleCenterViewModel.kt:76-83`
  - **问题说明**：真金购买路径（T-30071）标注 `// TODO: Full USD flow`，当前回退到 `purchase(autoRenew = true)`（钻石购买）。需要串联 `POST /payments/orders → launchBillingFlow → verify → POST /nobles/usd_purchase_intent` 完整真金链路。注：Server 端 `usd_purchase_intent` 端点也未实现，需与 Server 同步开发。
  - **修复建议**：与 Server T-00067 的 `usd_purchase_intent` handler 同步实现完整的 USD 购买链路。
...
1. **防腐层隔离完美**：`com.android.billingclient` 仅出现在 `GooglePlayBillingAdapter.kt:5` 一处。业务层（RechargeViewModel、RechargeScreen、AppContainer）均通过 `IBillingPort` 接口消费，无任何 Google Play Billing SDK 直接引用。

2. **DI 链路完整可追踪**：`AppContainer.fromBuildConfig()` → `GooglePlayBillingAdapter(appCtx)` → `RechargeScreen` factory → `RechargeViewModel(billingPort)` → `createOrderAndPay()` 完整流。`forUnitTest()` 路径注入 `FakeBillingPort()`。

3. **前置 6 项 P0 修复全部验证通过**：NobleRenewalListener WS 监听、RechargeViewModel BillingPort 调用、VerifyResponse 字段名、DevToolsScreen Repository 接口化、BillingClient 回调 SharedFlow 桥接、生产环境 GooglePlayBillingAdapter 注入 — 6 项均代码级验证正确。

4. **createOrderAndPay() 完整五步流**：connect → queryProductDetails → launchBillingFlow → collect purchaseResults → verify + acknowledge → disconnect。`PendingPurchaseHandler` 提供崩溃恢复容错机制。

5. **HTTP REST 核心支付路径 100% 匹配**：`payments/skus`、`payments/orders`、`payments/google/verify` 三条核心路径的 HTTP method、path 模板、request body 字段名（snake_case）与 App Server 路由注册完全一致。

6. **NobleBadge + NobleEntrancePlayer 组件实现完整**：徽章组件 `NobleBadge/NobleBadgeSmall` 6 档颜色映射正确，`EntranceQueueController` FIFO 队列防止叠加爆炸，Composable 动画使用 `slideInVertically + fadeIn` 良好。

7. **PendingPurchaseHandler 容错设计**：DataStore 持久化 pending purchase，`retryPending()` 在冷启动时自动重试，`maxRetries=3` 上限防无限循环。

8. **WalletViewModel WS 处理规范**：`handleWsMessage()` 使用 `JsonParser.parseString()` + `get("type")?.asString` JSON 解析方式，`CancellationException` 正确 re-throw。

---

#### 已知 P1 遗留（本轮维持非阻塞）

以下 6 项已在 Round 1~3 识别，本轮未修复，维持非阻塞状态：

- [ ] **P1-1**: `SharedFlow.collect` 无限监听 — 无生命周期感知取消机制
- [ ] **P1-2**: `NobleEntrancePlayer` Lottie 集成未完成
- [ ] **P1-3**: `NobleBadge` 硬编码颜色（`NOBLE_COLORS` 列表硬编码 6 档 hex，未从 Server `badge_color` 字段读取）
- [ ] **P1-4**: `MyNoble` 字段跨引用（`RetrofitNobilityRepository.getMyNoble()` 中 `tierName = ""`、`badgeColor = ""`、`entranceAnimationUrl = null`、`bgmUrl = null` 为占位值，需从 `tiers` 列表中查找匹配 tier 回填）
- [ ] **P1-5**: `DevToolsScreen` 按钮映射不完全对齐 TDS
- [ ] **P1-6**: `PendingPurchaseHandler` retry-count 无同步保护

---

**本轮结论**：❌ 存在 2 个 P0 级别问题（缺陷 1: BalanceDto 字段名不匹配导致 HTTP 余额永久为 0；缺陷 2: 4 个贵族 WS 信令 Android 端处理器完全缺失）。必须修复后重新审查。

*(文档头部状态机已修改为：`负责人 [TDD] | 状态 [❌ Failed]`)*

---

## 7. 附录: Android 源代码文件清单

优先级最高的源代码文件（供 global-code-reviewer 重点审查）：

### 支付模块核心链路
- `app/android/.../di/AppContainer.kt` — DI 容器（billingPort 注入点）
- `app/android/.../infra/billing/IBillingPort.kt` — 防腐层接口
- `app/android/.../infra/billing/GooglePlayBillingAdapter.kt` — BillingClient 封装
- `app/android/.../infra/billing/FakeBillingPort.kt` — 测试替身
- `app/android/.../recharge/RechargeScreen.kt` — 充值页 UI
- `app/android/.../recharge/RechargeViewModel.kt` — 充值 ViewModel
- `app/android/.../recharge/RechargeRepository.kt` — 充值数据层
- `app/android/.../recharge/data/SkuApi.kt` — SKU API
- `app/android/.../recharge/data/OrderApi.kt` — 订单 API
- `app/android/.../recharge/data/VerifyApi.kt` — 验证 API
- `app/android/.../recharge/data/PendingPurchaseHandler.kt` — 待处理购买恢复

### 贵族模块核心链路
- `app/android/.../noble/NobleCenterScreen.kt` — 贵族中心页
- `app/android/.../noble/NobleCenterViewModel.kt` — 贵族中心 ViewModel
- `app/android/.../noble/NobleRepository.kt` — 贵族数据层
- `app/android/.../noble/data/NobleApi.kt` — 贵族 HTTP API
- `app/android/.../noble/NobleBadge.kt` — 贵族徽章组件
- `app/android/.../noble/AvatarWithFrame.kt` — 头像框组件
- `app/android/.../noble/NobleEntrancePlayer.kt` — 进场特效播放器
- `app/android/.../noble/NobleNotifier.kt` — 续费/过期通知
- `app/android/.../noble/data/AckRepository.kt` — 提醒确认 API

---

### 【第 5 轮审查 — Round 5 Final Audit】
**@GlobalReview 审查意见：**
...
本轮为 Android Round 4 修复的最终验证审查。Round 4 发现 2 个 P0（BalanceDto 字段名错误 + 4 个贵族 WS 信号处理器缺失），TDD 声称全部修复。审计聚焦：

1. `WalletModels.kt` — BalanceDto `@SerializedName` 修复验证
2. `NobleRenewalListener.kt` — 6 个 WS 信号处理器完整性
3. `WsGsonAdapter.kt` — 贵族信号是否注册到 sealed class 多态分发
4. `RoomViewModel.kt` — 房间内贵族 WS 信号消费链
5. `NobleCenterScreen.kt` / `NobleCenterViewModel.kt` — 贵族中心页 WS 集成
6. `NobleEntrancePlayer.kt` — 进场特效组件集成链
7. `WsServerMessage.kt` — sealed class 是否有贵族类型

---

#### P0-1 修复验证 (BalanceDto)

- [x] **P0-1 (Round 4)**: **BalanceDto 字段名已正确修复** ✅

  | 检查项 | 文件:行号 | 状态 |
  |--------|----------|------|
  | `@SerializedName("diamond_balance")` | `WalletModels.kt:9` | ✅ 正确 |
  | Server 序列化为 `{"diamond_balance":...}` | `wallet/dto.rs` | ✅ 对齐 |
  | `RechargeViewModel.loadBalance()` 调用 `walletRepo.getBalance()` | `RechargeViewModel.kt:51` | ✅ 链路完整 |
  | `WalletViewModel` WS 路径读 `payload.get("diamond_balance")` | `WalletViewModel.kt:172` | ✅ 独立正确（WS 路径不受影响）|

  **验证结论**：P0-1 修复完整。HTTP 余额查询路径现在正确映射 `diamond_balance` 字段。

---

#### P0-2 修复验证 (6 个 WS 信号处理器) — ❌ **修复不完整，发现 4 项新 P0**

TDD 的修复方式是在 `NobleRenewalListener.kt` 中新增 4 个 `text.contains()` 分支（NobleRenewSuccess, NobleChanged, NobleEntered, NobleEntranceGlobal），使该文件名义上处理 6 个信号。但经过全链路审计，该修复存在系统性缺陷：

---

##### 新缺陷

- [ ] **缺陷 1**：[级别 P0] **NobleRenewalListener 是死代码 — 从未在任何 Composable 屏幕中组合使用**

  - **文件与行号**：`NobleRenewalListener.kt:30-161`（完整 Composable 函数定义）
  - **问题说明**：
    ```bash
    # 全局搜索 NobleRenewalListener 的调用方
    grep -rn "NobleRenewalListener\b" app/android/app/src/main/java/ --include="*.kt"
    # 结果：仅 NobleRenewalListener.kt 自身定义命中，零外部调用
    ```
    `NobleRenewalListener` 作为 `@Composable` 函数，必须被某个 Screen/Layout 组合才能启动其 `LaunchedEffect` 内的 `wsClient.state.collect` 协程。但没有任何 Screen 组合它：
    - `NobleCenterScreen.kt` — 不包含 NobleRenewalListener
    - `MainScreen.kt` — 仅导航到 NobleCenterScreen，不包含 NobleRenewalListener
    - `RoomScreen.kt` — 无贵族相关组件
    - 任何 Activity/Fragment — 均无调用

    **影响**：所有 6 个 WS 信号处理器（NobleRenewFailed, NobleExpired, NobleRenewSuccess, NobleChanged, NobleEntered, NobleEntranceGlobal）**永远不会被执行**。Server 发来的贵族 WS 信号在 Android 端全部静默丢弃。

  - **修复建议**：
    1. 在 `NobleCenterScreen` 中组合 `NobleRenewalListener`，传递 `wsClient`、`onNavigateToNobleCenter`、`onNobleChanged` (触发 `viewModel.loadMyNoble()`)
    2. 在 `RoomScreen`（或房内顶层 Composable）中组合 `NobleRenewalListener` 的另一个实例，传递 `onNobleEntered` 驱动 `EntranceQueueController`

  - **TDD 修复记录**：[等待 TDD 填写修复逻辑]

- [ ] **缺陷 2**：[级别 P0] **NobleEntrance 数据模型与 Server NobleEntered 协议 Payload 字段完全不匹配**

  - **文件与行号**：
    - Android: `NobleEntrancePlayer.kt:22-27` — `data class NobleEntrance(userId, tierLevel, tierName, badgeColor)`
    - Server: `app/server/src/room/handler/lifecycle.rs:277-291` — NobleEntered payload 实际字段

  - **问题说明**：

    | 维度 | Server NobleEntered Payload | Android NobleEntrance |
    |------|---------------------------|-----------------------|
    | user_id | ✅ `user_id` (String) | ❌ `userId` (无 @SerializedName) |
    | nickname | ✅ `nickname` (String) | ❌ 缺失 |
    | avatar_url | ✅ `avatar_url` (String) | ❌ 缺失 |
    | tier_id | ✅ `tier_id` (String) | ❌ 缺失（仅有 `tierLevel: Int`） |
    | level | ✅ `level` (Int) | ❌ 名称不同 (`tierLevel`) |
    | scope | ✅ `scope` (String) | ❌ 缺失 |
    | duration_ms | ✅ `duration_ms` (Int) | ❌ 缺失 |
    | entrance_animation_url | ✅ `entrance_animation_url` (String) | ❌ 缺失 |
    | bgm_url | ✅ `bgm_url` (String) | ❌ 缺失 |
    | marquee_text_en | ✅ `marquee_text_en` (String) | ❌ 缺失 |
    | marquee_text_ar | ✅ `marquee_text_ar` (String) | ❌ 缺失 |
    | frame_url | ✅ `frame_url` (String) | ❌ 缺失 |
    | badge_color | ✅ `badge_color` (String) | ❌ `badgeColor`（无 @SerializedName） |
    | tierName | ❌ 不存在 | ❌ `tierName: String`（Server 不下发） |

    Server 协议 §10.4.5 要求 NobleEntered.payload 为扁平结构（不允许嵌套在 `noble` key 下），含 13 个字段。Android `NobleEntrance` 仅有 4 个字段，且字段名均未加 `@SerializedName("snake_case")`。Gson 默认按字段名精确匹配反序列化，snake_case 字段将被全部跳过（设置为默认值）。

  - **修复建议**：
    1. 重构 `NobleEntrance` 为与 Server payload 一致的完整数据类：
    ```kotlin
    data class NobleEntrance(
        ...
        @SerializedName("entrance_animation_url") val entranceAnimationUrl: String?,
        @SerializedName("bgm_url") val bgmUrl: String?,
        @SerializedName("marquee_text_en") val marqueeTextEn: String,
        @SerializedName("marquee_text_ar") val marqueeTextAr: String,
        @SerializedName("frame_url") val frameUrl: String?,
        @SerializedName("badge_color") val badgeColor: String,
    )
    ```
    2. 同步更新 `NobleEntrancePlayer` 中读取新字段的代码

  - **TDD 修复记录**：[等待 TDD 填写修复逻辑]

- [ ] **缺陷 3**：[级别 P0] **Gson 反序列化无法工作 — 尝试将完整 WS 信封解析为 NobleEntrance**

  - **文件与行号**：`NobleRenewalListener.kt:81-82,87-88`
  - **问题说明**：
    ```kotlin
    // NobleRenewalListener.kt L81-82 (NobleEntered 分支)
    val entrance = gson.fromJson(text, NobleEntrance::class.java)
    // NobleRenewalListener.kt L87-88 (NobleEntranceGlobal 分支)
    val entrance = gson.fromJson(text, NobleEntrance::class.java)
    ```
    这里的 `text` 是完整的 WS 消息文本，其 JSON 结构为：
    ```json
    {"type":"NobleEntered","msg_id":"uuid","payload":{...},"timestamp":1234567890}
    ```
    而 `gson.fromJson(text, NobleEntrance::class.java)` 尝试将此顶层 JSON 反序列化为 `NobleEntrance(userId, tierLevel, tierName, badgeColor)`。Gson 会查找顶层 key `userId`、`tierLevel`、`tierName`、`badgeColor`，全部不存在 → 所有字段取默认值（`""`, `0`）。加之 catch 块静默吞错（`} catch (_: Exception) {}`），解析失败完全不可见。

    正确做法应是先解析顶层 JSON 提取 `payload` 对象，再对 payload 反序列化为 NobleEntrance：
    ```kotlin
    val root = com.google.gson.JsonParser.parseString(text).asJsonObject
    val payload = root.getAsJsonObject("payload") ?: return
    val entrance = gson.fromJson(payload, NobleEntrance::class.java)
    ```

  - **修复建议**：先解析 WS 信封提取 `payload`，再反序列化 payload 为领域模型。同时将 `catch` 块改为至少打 `Log.w`，方便排查问题。

  - **TDD 修复记录**：[等待 TDD 填写修复逻辑]

- [ ] **缺陷 4**：[级别 P0] **贵族 WS 信号未注册到 `WsServerMessageTypeAdapter` — RoomViewModel 内所有贵族信号被丢弃为 Unknown**

  - **文件与行号**：`WsGsonAdapter.kt:67-101`（`WsServerMessageTypeAdapter.deserialize()` 的 `when` 分发）
  - **问题说明**：
    RoomViewModel 通过 `WsGsonFactory.create().fromJson(raw, WsServerMessage::class.java)` 解析所有 WS 消息。`WsServerMessageTypeAdapter` 的 `when(type)` 分发块不含任何贵族信号类型（NobleEntered, NobleChanged, NobleEntranceGlobal, NobleRenewSuccess, NobleRenewFailed, NobleExpired, BalanceUpdated）。这些信号全部落入 `else -> WsServerMessage.Unknown(type)` 分支。
    
    当前已知的缓解措施是：
    - `WalletViewModel` 和 `GiftPanelViewModel` 各自独立监听 WS state 并手动 `JsonParser.parseString` 解析 BalanceUpdated（绕过 sealed class）
    - NobleRenewalListener 也是独立监听 WS state（但本题报告的缺陷 1~3 使其同样无法工作）
    
    **影响**：即便 NobleRenewalListener 被正确组合且数据模型修复，RoomViewModel 内部对 NobleEntered/NobleChanged 等房间级信号的消费链仍然断裂。RoomViewModel 的 `when(msg)` 分支需要新增贵族信令处理以驱动 NobleEntrancePlayer 和观众席贵族徽章更新。

  - **修复建议**（分层修复）：
    1. **WsServerMessage.kt**: 新增贵族 sealed class 子类型（`NobleChanged`, `NobleEntered`, `NobleEntranceGlobal` 等），参照 `UserJoined` 模式（嵌套 payload 类 + `@SerializedName` 注解）
    2. **WsGsonAdapter.kt**: 在 `when(type)` 中注册新类型的分发
    3. **RoomViewModel.kt**: 在 `when(msg)` 中新增贵族信号的分支处理（NobleEntered → 入队 EntranceQueueController, NobleChanged → 刷新成员 noble 字段）
...
- [ ] **缺陷 5**：[级别 P1] **NobleEntrancePlayer 同样从未在任何 Screen 中组合使用**

  - **文件与行号**：`NobleEntrancePlayer.kt:36-58`（Composable 函数定义），全项目零调用
  - **问题说明**：`NobleEntrancePlayer` 组件已实现（FIFO 队列 + slideInVertically 动画），但未在任何 Screen 的 Composable 树中组合。即使未来 NobleEntered WS 信号被正确解析并投递给 `EntranceQueueController`，也没有 UI 层渲染进场特效。
  - **修复建议**：在 `RoomScreen` 的 Composable 层级中组合 `NobleEntrancePlayer`，绑定到 `EntranceQueueController.current` 状态。

  - **TDD 修复记录**：[等待 TDD 填写修复逻辑]

- [ ] **缺陷 6**：[级别 P1] **NobleCenterViewModel 不监听 NobleChanged WS 信号**

  - **文件与行号**：`NobleCenterViewModel.kt`（全文搜索 NobleChanged/wsClient 零命中）
  - **问题说明**：用户购买/升级/降级贵族后，Server 发送 NobleChanged S→C 单播。但 NobleCenterViewModel 不订阅 WS 消息流，用户需手动退出重进贵族中心页才能看到最新状态。
  - **修复建议**：在 NobleCenterViewModel 中注入 IWebSocketClient 或通过 NobleRenewalListener 的 `onNobleChanged` 回调触发 `loadMyNoble()` 刷新。
...
1. **P0-1 修复完整**：BalanceDto `@SerializedName("diamond_balance")` 与 Server `diamond_balance` 字段对齐。
2. **防腐层隔离完美**：`com.android.billingclient` 仅出现在 `GooglePlayBillingAdapter.kt` 一处。
3. **DI 链路完整**：`AppContainer → RechargeScreen → RechargeViewModel → BillingPort` 完整可追踪。
4. **前置 6 项 R1-R3 P0 修复验证通过**。
5. **`./gradlew :app:compileLocalDebugKotlin` BUILD SUCCESSFUL**。

---

#### 已知 P1 遗留（本轮维持非阻塞，同 Round 4）

- P1-1: SharedFlow.collect 无限监听 — 无生命周期取消机制
- P1-2: NobleEntrancePlayer Lottie 集成未完成
- P1-3: NobleBadge 硬编码颜色
- P1-4: MyNoble 字段跨引用
- P1-5: DevToolsScreen 按钮映射不完全对齐 TDS
- P1-6: PendingPurchaseHandler retry-count 无同步保护
...
**本轮结论**：❌ 存在 4 个新 P0 级别问题。P0-2（Round 4 的 4 个缺失 WS 信号处理器）名义上在 NobleRenewalListener.kt 中添加了代码分支，但全链路审计发现：

1. **NobleRenewalListener 未被任何 Screen 组合**（P0-1 新）→ 死代码
2. **NobleEntrance 数据模型与 Server payload 字段不匹配**（P0-2 新）
3. **Gson 反序列化目标错误**（P0-3 新）— 将 WS 信封当领域模型解析
4. **贵族信号未注册到 WsServerMessageTypeAdapter**（P0-4 新）→ RoomViewModel 丢弃所有贵族信号

以上 4 项任意一项单独存在即可导致贵族 WS 信号处理链路完全断裂；4 项叠加意味着 Server 发送的 6 个贵族 WS 信号在 Android 端**全部静默丢弃**。必须系统性修复后方可放行。
