...
test.describe('TC-GOVERNANCE WEB - 房间治理日志', () => {
  ...
  test('TC-GOVERNANCE-00001: 治理日志页 - 筛选 + 分页 + 空状态', async ({ page, e2eEnv }: any) => {
    ...
    // Step1：访问 /rooms/governance
    await page.goto('/rooms/governance');
    await page.waitForLoadState('networkidle');
    await agent.aiAssert('页面标题包含"治理日志"，页面显示筛选区域和表格（或空状态提示）');

    // Step2：验证表格列（当页面有数据时验证列标题；空状态时跳过）
    const hasTableData = await agent.aiBoolean('表格中是否有数据行（非空状态）？');
    if (hasTableData) {
      await agent.aiAssert('表格包含时间/类型/房间/操作者/目标用户等主要列');
    } else {
      // 空状态验证：表格列头仍应可见
      const hasColumnHeaders = await page.locator('table thead').count().then(c => c > 0).catch(() => false);
      if (hasColumnHeaders) {
        console.log('[TC-GOVERNANCE-00001] 表格显示空状态，但列头存在，通过验证');
      } else {
        console.log('[TC-GOVERNANCE-00001] 暂无治理日志数据，跳过列验证');
      }
    }
    ...
    await page.waitForLoadState('networkidle');
    const showsEmptyState = await agent.aiBoolean('列表是否显示空状态（"暂无记录"或 No Data 插画）？');
    if (showsEmptyState) {
      await agent.aiAssert('空状态包含插画和"暂无记录"文案');
    }

    // Step6：清除所有筛选，验证恢复默认
    await page.goto('/rooms/governance');
    await page.waitForLoadState('networkidle');
    await agent.aiAssert('清除筛选后，治理日志页恢复默认状态（列表显示或空状态提示，页面布局正常）');
  ...
  test('TC-GOVERNANCE-00003: 用户详情联动跳转 + 权限控制', async ({ page, e2eEnv }: any) => {
    ...
    if (UID) {
      ...
      if (hasGovernanceLink) {
        ...
        await page.waitForURL(/rooms\/governance/, { timeout: 20_000 });
        await page.waitForLoadState('networkidle');
    ...
    await page.goto('/login');
    await page.waitForLoadState('networkidle');
    await agent.aiAction('在用户名输入 "e2e_cs"，密码输入 "admin_password_change_me"，点击登录（或其他低权限账号）');
    // 不管登录成功与否，直接访问 /rooms/governance
    await page.goto('/rooms/governance');
    await page.waitForLoadState('networkidle');
