...
2. **Plan 阶段 (方案设计)**：从 `doc/Tasks.md` 领取任务，读取 `doc/protocol.md` (通信契约)，并从对应端的 `doc/xxx_arch/index.md` 顺藤摸瓜读取相关子文档，输出具体的技术实现方案与文件修改清单。
3. **TDD 阶段 (测试驱动编码)**：先写测试用例，运行报错后，再实现业务代码，直到测试完全通过。
4. **DoD 阶段 (状态与文档同步 - 绝对红线)**：代码开发和测试完成后，**必须主动执行**：
   - 找到对应端的文档目录（如 `doc/web_arch/`），更新具体受影响的子模块文档（如新增了路由，则更新 `router.md`；若新增了全新模块，必须同步更新 `index.md` 的索引）。
...
**⚠️ 寻路铁律：对于目录级文档（如 `doc/server_arch/`），必须先读取其内部的 `index.md` 获取结构地图，再精准定位读取目标子文档，严禁盲猜文件名。**
...
- **Rust Server 架构入口？** -> 从 `doc/server_arch/index.md` 开始寻路
- **Web 端架构入口？** -> 从 `doc/web_arch/index.md` 开始寻路
- **Android 端架构入口？** -> 从 `doc/android_arch/index.md` 开始寻路
