...
| **T-30044** | Android | Governance | 禁麦/禁言 UI 反馈 + 抱麦集成 [TDS](./tds/android/T-30044.md) | T-00029, T-00030, T-30042 | 自身被禁麦时麦位"+"按钮置灰且点击 Toast；自身被禁言时聊天输入框 enabled=false + 占位文本 "你已被禁言 N 分钟"；被 `ForceTakeMic` 时自动请求麦克风权限，成功则进麦，拒绝则自动发 MicLeave；被 `ForceLeaveMic` 时自动停止推流显示 Toast "你已被抱下麦" | 1. 禁麦用户点麦位"+" 无网络请求<br>2. 禁言用户输入框 disabled 且提交按钮置灰<br>3. 被抱上麦后如未授权麦克风，自动完成 MicLeave<br>4. 抱下麦后本地麦元状态同步为离线<br>5. `Key('chat_input')` disabled 可测 | Review | 6h | **DoD** | [T-30044.md](./design/android/T-30044.md) |
