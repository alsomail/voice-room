...
|---------|--------|------|----------|----------|----------|-------------|----------|------------|----------|---------------------|------------------|----------------------|
| **T-00100** ⭐ | 基建 | Infra/Protocol | **协议字段全量冻结 — 三协议层 schema 落锚** [TDS](../tds/infra/T-00100.md) | T-0000V | (a) `protocol/conventions.md` 新增 §4 snake_case 强制 / §5 WS payload 嵌套 / §6 envelope 双 ID 三铁律；(b) `websocket_signals.md` 补齐 21+ 缺失信令；(c) HTTP `auth/room/admin/ranking_api.md` 全 endpoint 字段表；(d) `providers.md` **新增** Redis Pub/Sub `admin:events` 章节（首次落锚）；(e) `schemas/{ws,http,pubsub}/*.schema.json` 机器可读契约；(f) `mic_slots` 强类型化 | 1. 8 项 PROTO-FREEZE 用例全过<br>2. ajv compile schemas 全通过<br>3. server 出栈抽样 envelope 与 schema 0 偏差<br>4. ADR-0003 沉淀决策 | 20h | Plan | Todo | - | - | - |
| **T-00101** | Android | data/remote/ws | Android sealed class 反序列化 + RoomViewModel 27+ 野生分支全量迁移 [TDS](../tds/android/T-00101.md) | T-00100 | (a) `WsServerMessage`/`WsClientMessage` sealed class 28 子类型；(b) RuntimeTypeAdapterFactory；(c) RoomViewModel `handleWsMessage` 全量重写消灭 `json.get()`；(d) 集成 OkHttpWebSocketClient；(e) 单测 fixture 由 schemas/ 校验；(f) `?: return` 静默吞错改为 ERROR + 埋点 | 1. PROTO-1~6 + SCHEMA-1 全过<br>2. RoomViewModelTest 全量重写通过<br>3. grep `json.get` 在 RoomViewModel 0 命中 | 14h | Dod | ✅ Done | - | - | - |
| **T-00102** | Web | api | Web Admin Zod schema 接入（HTTP DTO 强校验） [TDS](../tds/web/T-00102.md) | T-00100 | apiClient 拦截器加 Zod parse；schema 由 `json-schema-to-zod` 一次性 codegen 入仓 | 1. ZOD-1~4 全过<br>2. 全部现有 endpoint 覆盖率 100% | 8h | Plan | Todo | - | - | - |
| **T-00103** | App Server | ws / dto | App Server 出栈 envelope schema 自检 + HTTP DTO `deny_unknown_fields` [TDS](../tds/server/T-00103.md) | T-00100 | dev/test profile WS 出栈 schema_guard panic；prod ERROR + 埋点；Request DTO 加 `deny_unknown_fields`；ping/pong 兼容期双发 | 1. GUARD-1/2 + DENY-1 + PING-COMPAT 全过<br>2. 现有集成测试 0 回归 | 5h | Dod | Done | - | - | - |
| **T-00104** ⭐ | 基建 | Infra/QA | Android × Server 跨语言 WS 回环测试（8 场景）[TDS](../tds/infra/T-00104.md) | T-00100, T-00101, T-00103 | 真 server 进程 + 真 Android Instrumented Test 跑 Ping/Pong, JoinRoom→UserJoined, TakeMic→MicTaken, LeaveMic→MicLeft, SendMessage→RoomMessage, SendGift→GiftReceived+BalanceUpdated, MuteUser→UserMuted, KickUser→UserLeft 8 场景；schema 双向校验；CI 夜间跑 | 1. 8 场景各 ≥3 次稳定通过<br>2. CI workflow 落地 | 12h | Plan | Todo | - | - | - |
...
| **T-00106** | 基建 | Infra/Protocol | 协议字段级 AST 审计（升级 T-0000T）+ CI 阻塞 [TDS](../tds/infra/T-00106.md) | T-00100, T-0000T | 解析 server `json!` 宏 / Android `@SerializedName` / Web Zod；与 schemas/ 字段集合比对；mismatch 报 P0 阻塞 PR | 1. FIELD-1~3 + CI-1 全过<br>2. T-0000T 53 tests 0 回归 | 10h | Dod | ✅ Done | - | - | - |
| **T-00107** | 基建 | Infra/Protocol | 历史 TDS 字段表全量回填（升级 T-0000U）[TDS](../tds/infra/T-00107.md) | T-00106 | 借助 T-00106 字段级审计扫描，按优先级回填字段表 schemas/ 锚点 | 1. 字段级审计 0 缺失<br>2. 批次记录入档 | 12h | Plan | Todo | - | - | - |
| **T-00108** | 基建 | Infra/Protocol | `ping`/`pong` → `Ping`/`Pong` 三端同步升级 [TDS](../tds/infra/T-00108.md) | T-00100 | server 出/入栈兼容期双向；Android 心跳改发 PascalCase；fixture 全量改；Web 检查 | 1. PING-1~3 + GREP-1 全过<br>2. 现有心跳测试 0 回归 | 4h | Dod | ✅ Done | - | - | - |
...
**汇总**：29 个 Task（原 20 + Phase 1.7-extended 9 个），预估总工时 **170 人时（≈21.2 人天）**。**模块 9 完成进度：21/29（T-00100~T-00108 9 个新 Task 中 T-00101/T-00102/T-00103/T-00105/T-00106/T-00108 6 个已 Done）**。
