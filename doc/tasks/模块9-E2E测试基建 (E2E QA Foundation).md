...
| **T-0000N** | 基建 | Infra/E2E | AppServer / AdminServer 暴露统一 `/health` 端点 [TDS](../tds/infra/T-0000N.md) | T-0000H, T-0000M | 两服务暴露轻量 `/health`（200 探活，纯静态 JSON），让 `e2e-up.sh` `wait-on http-get://...:3000/health` 与 `preflight.sh` 5 端健康检查真生效；当前两端仅有 `/ping`（`grep '"/health"' app/server/src` 0 命中），是 T-0000H 起的预存在缺陷 | 1. AppServer GET /health 返回 200 + `{status:"ok", service:"app-server", version}`<br>2. AdminServer GET /health 返回 200 + `service:"admin-server"`<br>3. `npm run e2e:up` 5 端 wait-on 全绿<br>4. `npm run preflight` 1 秒内绿表 | 1 | Dod | ✅ Done | [✅ Passed](../review/模块9-E2E测试基建.md) | [✅ Passed](../../tests/report-20260428-154125/API/TC-INFRA/Report.md) TC-INFRA-00003✅ preflight 5/5✅ | ✅ Released |
| **T-0000O** | 基建 | Infra/E2E | ranking_test::r08 perf flake known-issue 收口 [TDS](../tds/infra/T-0000O.md) | T-0000M | `ranking_test::r08_response_time_under_100ms` 在 dev 机器上 p95 偶超 100ms（实测 315ms）；触发条件：与其它 DB 测试并发 + 冷连接池 warm-up 抖动；本 Task 在测试函数加 `#[ignore]`，建立 `doc/tests/known-issues.md` 登记册，长期方向迁独立 perf 套件 | 1. r08 加 `#[ignore = "perf flake; tracked by T-0000O"]`<br>2. `doc/tests/known-issues.md` 首条记录覆盖 5 必填字段（现象/触发条件/规避/手动跑命令/长期方向）<br>3. `cargo test -p voice-room-server` 默认 0 fail 且不含 r08 输出；`-- --ignored --test-threads=1` 单跑稳定通过<br>4. RUNBOOK 故障排查链向 known-issues.md | 1 | Dod | ✅ Done | [✅ Passed](../review/模块9-E2E测试基建.md) | [✅ Passed](../../tests/report-20260428-154125/API/TC-RANKING/Report.md) TC-RANKING-00003✅ (p95≤100ms) | ✅ Released |
| **T-0000P** | 基建 | Infra/E2E | Midscene env 注入链补齐（MIDSCENE_MODEL_API_KEY → AI runner） [TDS](../tds/infra/T-0000P.md) | T-0000H | envLoader 增加 Midscene env 读取（`MIDSCENE_MODEL_API_KEY` fallback `OPENAI_API_KEY`）+ .env.example 补充注释 + CI workflow 注入 Secret | 1. 本地设置 MIDSCENE_MODEL_API_KEY 后 WEB 用例正常执行<br>2. 缺失时打印警告并 skip（不阻塞 API/INFRA 测试）<br>3. CI 注入 Secret 后 WEB 用例非 skip<br>4. 示例文件含 3 种部署形态注释 | 2h | Dod | Done | [✅ Passed](../review/模块9-E2E测试基建.md) | [✅ Passed](../../tests/report-20260429-024556/SUMMARY.md) WEB 16/18 PASS（T-0000R 闭环后 9 FAIL 全绿；2 SKIP 为环境前置） | [✅ Released](../../tests/report-20260429-024556/SUMMARY.md)（T-0000R/S 闭环后升回 ✅） |
| **T-0000Q** | 基建 | Infra/E2E | docker compose preflight 端口冲突检测扩展 [TDS](../tds/infra/T-0000Q.md) | T-0000G | e2e-up.sh 增加端口检测（5432/6379/3000/3001/5173），冲突时打印彩色错误 + 占用进程 PID/名称 + kill 命令提示 | 1. 端口空闲时正常启动<br>2. 冲突时红色错误 + kill 命令提示 + 退出码 1<br>3. 跨平台（macOS lsof / Linux ss）<br>4. 错误信息含进程名称 | 2h | Dod | ✅ Done | [✅ Passed](../review/模块9-E2E测试基建.md) | [✅ Passed](../../tests/report-20260428-154125/API/TC-INFRA-Q/Report.md) I-1✅ | ✅ Released |
| **T-0000R** | 基建 | Infra/E2E | WEB E2E 9-FAIL 修复（TC-ROOM/GIFT/USER 测试代码/数据/时序硬化） [TDS](../tds/infra/T-0000R.md) | T-0000P | 来自 v3 战报（report-20260428-154125）9 个 WEB FAIL 全部为测试侧问题：① TC-GIFT-00002 chromium timeout + firefox AI prompt（toggle 而非文字）；② TC-ROOM-00002 测试数据不足 10 房间；③ TC-ROOM-00003 ant-modal 双标题 strict mode；④ TC-USER-00002 webkit 抽屉时序。**不动业务代码** | 1. WEB 套件 54/54 PASS<br>2. seed ≥12 房间幂等<br>3. locator 唯一锁定 `.ant-modal-confirm-title`<br>4. webkit 抽屉关闭等待后再 AI 断言连续 5 次稳定<br>5. `git diff -- app/` 0 行 | 6h | Dod | ✅ Done | [✅ Passed](../tds/infra/T-0000R.md) Round 2 通过（TDS §5） | [✅ Passed](../../tests/report-20260429-024556/SUMMARY.md) WEB 16/16 PASS（原 9 FAIL 全绿） | ✅ Released |
| **T-0000S** | 基建 | Infra/E2E | E2E fixture token + redis-cli 容器化自动注入（解锁 26/29 SKIP-KNOWN） [TDS](../tds/infra/T-0000S.md) | T-0000H, T-00041, T-00042 | globalSetup 自动调 `/auth/test-login` 注册 OP/USER_B/MUTED 三个角色并注入 `process.env`；redis-cli 改走 `docker exec voice-room-redis-1 redis-cli`；TC-AUTH 13 + TC-WS 3 + TC-RANKING 1 + TC-MIC 6 + TC-CHAT 2 + TC-GIFT 1 共 26 用例自动跑通；剩余 3（INFRA Docker×2 + INFRA-Q×1）登记 follow-up | 1. `npm run e2e:local -- API/` 输出 75 passed / 3 skipped<br>2. JWT decode 三 token role/subject/muted 字段正确<br>3. 本地无 redis-cli 时 redis 类用例仍 PASS<br>4. globalSetup 重跑 5 次幂等<br>5. CI workflow 无 token 条件分支<br>6. `git diff -- app/` 0 行 | 5h | Dod | ✅ Done | [✅ Passed](../review/模块9-E2E测试基建.md) | [✅ Passed](../../tests/report-20260429-024556/SUMMARY.md) API 225/234 PASS（原 26 SKIP-KNOWN 全部解锁），WEB 16/18 PASS；附 2 自愈 fix（TC-WS-00006 DELETE / TC-CHAT-00001 监听器竞态） | ✅ Released |
